...
import com.thiefspin.jwt.Algorithm._
...
object JWT {
   ...
   * @return String
   */
  def encode(secret: String, payload: JsValue, header: JsObject = Json.obj(), algorithm: Option[Algorithm] = Some(Algorithm.HS256)): String = {
      ...
      decodeParts(jwt, key.isDefined) match {
        case Success((headerJs, payload, signature, signingInput)) =>
          validateJWTHeader(headerJs) match {
            ...
            case Some(header@_) if key.isDefined =>
        ...
        case Failure(_: JWTException.NotEnoughSegments) =>
          JWTResult.NotEnoughSegments
        case Failure(_: JWTException.TooManySegments) =>
          JWTResult.TooManySegments
        case Failure(e) => JWTResult.JWTError(e)
      }
    }
  }
   ...
   * @return String
   */
  private[jwt] def encodePayload(payload: JsValue): String =
