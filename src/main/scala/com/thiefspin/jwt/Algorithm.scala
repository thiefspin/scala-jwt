...
object Algorithm {
  ...
  implicit def format: Format[Algorithm] = Format(reads, writes)

  private val reads: Reads[Algorithm] = new Reads[Algorithm] {
  ...
  private val writes: Writes[Algorithm] = new Writes[Algorithm] {
    override def writes(alg: Algorithm): JsValue = JsString(alg.name)
  }
